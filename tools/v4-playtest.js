...
(async () => {
  ...
  const snap = () => ev(`(function(){var S=${G}.S,d=document,q=function(s){return !!d.querySelector(s);},ok=function(id){var e=d.getElementById(id);return !!(e&&!e.disabled);};
    ...
    return o;})()`);
  let loops = 0, lastEra = 0, eraAt = {}, lastCkpt = 0, mixerEvery = 0, visited6 = false, visitedOp = false, wantedFocus = null, lastNote = '';
  ...
  while ((Date.now() - t0) / 60000 < MAXMIN) {
    ...
    // ---------------- STATISTICAL ----------------
      ...
      if (E.focus !== 'auto') { const want = E.gap > 0.18 ? 'generalize' : (E.methods < 6 && E.survey < 60 && s.data < 300 ? 'explore' : 'fit'); if (want !== E.focus && want !== wantedFocus) { await click('#foc-' + want, 'focus'); wantedFocus = want; } }
      if (s.dom.okBuys.includes('buy-dataset') && E.dataset < 22) await click('#buy-dataset');
      if (s.dom.okBuys.includes('buy-model') && E.model < 16 && s.data > 200) await click('#buy-model');
      if (s.dom.xpBadge && +s.dom.xpBadge > 0 && loops % 6 === 0 && !s.dom.drawer) { await click('#xpBtn', 'experiments'); await sleep(500); const m = await ev(`(function(){var b=document.getElementById('xc-buy-method');if(b&&!b.disabled)return 'xc-buy-method';var u=document.querySelector('.xc-buy:not(:disabled)');return u?u.id:null;})()`); if (m) { await click('#' + m, 'fund'); log('funded ' + m); await beat('statistical-experiments'); } await sleep(300); await click('#researchClose'); }
