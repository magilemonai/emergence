...
| 2 | Symbolic | Theorem-proving as a process: Rulesets emit Inference, aimed at one proof at a time; repeatable Optimization + Inference-Capacity lemmas; Compile→Axioms prestige; proven-theorems row | **CRT phosphor terminal** — VT323 + IBM Plex Mono, green phosphor, scanlines + curved glass + flicker, CRT bezel screen, schematic backdrop, terminal-console hero (boot-glyph + `> write rule` prompt + blinking caret), mechanical-key sound | **Built; world-pass feature-complete** (all 8 levers; pacing retuned to 5-6m; awaiting ChatGPT critique) |
