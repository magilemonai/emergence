...
- Run-2 alterations (WO-10 proposals, exact): (1) app.js boot: read localStorage[LEGACY_KEY] in try/catch → createSim({legacy}) AND applyToFresh(sim.state, legacy) before first render; after world.setHud(hud) call installLegacyFx({world, sim, doc}) and its sync() on era change; at the ending write fromRun(sim.state, sim.state.legacy). (2) Title cards: when flags.run2 the subtitle reads from legacy (v4: name + ' remembers', 'run N'); words in engine/voice. (3) Origins commission signed '· NAME' when flags.run2 && legacy.name (engine commissionTick + view). (4) Optional cfg.e2.legacyRules: rules present at Symbolic open on run 2. Wire `fx.operated`/`world.operated` + body class into the shell's era switch after WO-11.
